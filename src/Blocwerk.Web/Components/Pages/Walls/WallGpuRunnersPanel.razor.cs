...
public partial class WallGpuRunnersPanel : IAsyncDisposable
{
    ...
    private string CudaCommand => GpuRunnerCommands.Cuda(ServerUrl);

    private string VulkanCommand => GpuRunnerCommands.Vulkan(ServerUrl);

    private string NativeCommand => GpuRunnerCommands.Native(ServerUrl);

    private static string EnvLine(string key) => GpuRunnerCommands.EnvLine(key);
