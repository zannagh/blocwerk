...
function segments(list, facets, lift, dimmed) {
    const pos = [];
    const col = [];
...
export function buildOutlines(lit, dim, facets, lift) {
    const group = new THREE.Group();
    group.add(new THREE.LineSegments(segments(lit, facets, lift, false),
        new THREE.LineBasicMaterial({ vertexColors: true })));
    if (dim.length) {
        group.add(new THREE.LineSegments(segments(dim, facets, lift, true),
