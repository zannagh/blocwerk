    ...
    TimeProvider? clock = null)
{
    ...
    public async Task HelloAsync(GpuRunner runner, RunnerHello hello, CancellationToken ct)
    {
        ...
        row.Trainer = hello.Trainer?.Trim().ToLowerInvariant() is "gsplat" or "brush" ? hello.Trainer.Trim().ToLowerInvariant() : null;
    ...
    public async Task<GpuJob> EnqueueAsync(GpuJob job, CancellationToken ct)
    {
        ...
        job.CreatedAt = Now;
        job.Stage = "waiting for a 3D runner";
        db.GpuJobs.Add(job);
        await db.SaveChangesAsync(ct);
        logger.LogInformation(
    ...
    /// <summary>
    /// The highest quality the server lets a runner claim: what it reported, but ultra only for gsplat with at least
    /// 12 GB (else max); unknown names are dropped (the runner then counts as <c>high</c>).
    /// </summary>
    internal static string? AcceptedMaxQuality(string? reported, string? trainer, int? vramMb) =>
        CaptureSplatDocuments.ParseQuality(reported) switch
        {
            null => null,
            SplatQuality.Ultra when trainer != "gsplat" || vramMb is not >= UltraMinVramMb => "max",
            { } q => CaptureSplatDocuments.QualityName(q),
        };
