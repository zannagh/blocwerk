...
public static class WallFrameRegistration
{
    ...
    /// <summary>The fit is refused when the kept markers still miss by more than this (RMS, mm).</summary>
        ...
        WallGeometryDocument reference, WallGeometryDocument solved, IReadOnlySet<int>? eligibleIds)
    {
        ...
        var rms = Math.Sqrt(residuals.Values.Average(r => r * r));
        if (rms > MaxRmsMm)
        {
            return Refuse(
        ...
        return new FrameRegistrationResult(true, fit, kept, changed, outliers, rms, residuals.Values.Max(), null);
    }
...
/// <param name="RmsMm">RMS corner residual of the used markers after the fit.</param>
