...
/// <summary>
/// Filesystem-backed <see cref="IWallPhotoMasterStorage"/> rooted at
/// <c>WallPhotoMasterSettings.StoragePath</c>. Temp files live under a <c>tmp</c> subfolder of the
/// same root so a commit is a plain rename, and the traversal guard in
/// <see cref="ResolvePhysicalPath"/> is identical to the wall-image store's.
/// </summary>
public class FileSystemWallPhotoMasterStorage : IWallPhotoMasterStorage
{
    private readonly string root;
    private readonly string tempRoot;

    public FileSystemWallPhotoMasterStorage(BlocwerkSettings settings)
    {
        root = Path.GetFullPath(settings.WallPhotoMaster.StoragePath);
