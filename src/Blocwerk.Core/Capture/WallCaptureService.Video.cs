...
public sealed partial class WallCaptureService
{
    ...
    private async Task<Guid> EnsureVideoAllowedAsync(Guid captureId)
    {
        ...
        await using (db)
        {
            ...
            {
                throw new UserFacingException("The photo-real view is not set up on this server, so a video would not be used.");
            }
    ...
    {
        var path = files.ResolvePhysicalPath(stored) ?? throw new UserFacingException("The video could not be stored.");
        ...
        catch (InvalidDataException ex)
        {
            ...
            throw new UserFacingException($"{name ?? "The video"} could not be read as a video.");
        }
