...
(plain gsplat MCMC; with the opt-in wall zones 30k steps up to 3 M, zoned()).
...
PROFILES = {
    ...
    # photos at the ingest cap (INGEST_MAX_EDGE, 4096), 50k steps, up to 6 M splats: plain gsplat MCMC.
...
# ultra with the opt-in wall zones (SPLAT_WALL_ZONES=1 / options.wallZones; zoned() below): the cap goes to the
# wall, so on The Attic 3 M at 30k steps beat 6 M at 50k on the held-out wall PSNR (README, "Wall zones").
ZONED_ULTRA = {"steps": 30000, "max_splats": 3_000_000, "min_splats": 1_500_000}


def zoned(profile):
    """The profile a wall-zone training uses: ultra at its defaults (no maxSteps override) -> ZONED_ULTRA."""
    base = PROFILES["ultra"]
    if profile.name == "ultra" and profile.steps == base.steps and profile.max_splats == base.max_splats:
        return replace(profile, **ZONED_ULTRA)
    return profile
