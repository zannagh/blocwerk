...
Posts the latest progress at most every MIN_INTERVAL_S when it changed, and at least every
MAX_INTERVAL_S regardless (the server's 5-minute lease). A `cancel` answer or a 404/409/410 sets
`stop`, which kills Brush (procs.ToolRun's stop event)."""
import logging
import threading
import time

from .client import Gone, Transient, Unauthorized
...
class Heartbeat:
    def __init__(self, client, job_id, stop, clock=time.monotonic):
        self.client, self.job_id, self.stop, self.clock = client, job_id, stop, clock
