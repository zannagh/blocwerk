...
public class GpuRunnerResultTests
{
    ...
    public async Task Result_OverTheCap_IsRefused_AndNothingIsKept()
    {
        ...
        var outcome = await f.Queue.AcceptResultAsync(runner, job.Id, new MemoryStream(RunnerFixture.SlimPly(100)), null, CancellationToken.None);
    ...
    public async Task Result_ThatIsNotASplat_IsRefused(string body)
    {
        ...
        var outcome = await f.Queue.AcceptResultAsync(runner, job.Id, new MemoryStream(Encoding.ASCII.GetBytes(body)), null, CancellationToken.None);
    ...
    public async Task ValidResult_CompletesTheJob_AndHandsTheCaptureBackToThePipeline()
    {
        ...
        var outcome = await f.Queue.AcceptResultAsync(
            runner, job.Id, new MemoryStream(RunnerFixture.SlimPly()), "{\"steps\":5000,\"runnerGpu\":\"Apple M4\"}", CancellationToken.None);
